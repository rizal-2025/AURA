"""Secret-safe PostgreSQL preflight for the Windows staging credential."""
...
EXPECTED_DATABASE = "aura_demo_staging"
EXPECTED_USER = "aura_staging_runtime"
...
class PostgreSQLStagingPreflightError(RuntimeError):
    """A stable error whose text never includes configuration or driver data."""


def build_staging_database_url() -> URL:
...
def validate_staging_database_url(value: object) -> URL:
    if not isinstance(value, str) or not value or value != value.strip():
        raise PostgreSQLStagingPreflightError("AURA_STAGING_DATABASE_URL_INVALID")
    try:
        parsed = make_url(value)
        host = (parsed.host or "").casefold()
        port = parsed.port
    except (ArgumentError, AttributeError, TypeError, ValueError):
        raise PostgreSQLStagingPreflightError(
            "AURA_STAGING_DATABASE_URL_INVALID"
        ) from None
    if (
        ...
        raise PostgreSQLStagingPreflightError(
            "AURA_STAGING_DATABASE_TARGET_INVALID"
        )
    return parsed
...
def _database_access(connection) -> dict[str, bool]:
    rows = connection.execute(
        text(
            ...
            "WHERE datname IN ('aura_test', 'aura_demo_public')"
        )
    ).all()
...
def run_preflight() -> None:
    ...
    if os.environ.get("APP_ENV") != "demo":
        raise PostgreSQLStagingPreflightError("AURA_STAGING_APP_ENV_INVALID")
    if not os.environ.get("PGPASSFILE"):
        raise PostgreSQLStagingPreflightError("AURA_STAGING_PGPASSFILE_MISSING")

    parsed = validate_staging_database_url(value)
    ...
    try:
        ...
        raise PostgreSQLStagingPreflightError(
            "AURA_STAGING_DATABASE_CONNECTION_FAILED"
        ) from None
    finally:
    ...
    if current_database != EXPECTED_DATABASE or current_user != EXPECTED_USER:
        raise PostgreSQLStagingPreflightError(
            "AURA_STAGING_DATABASE_IDENTITY_INVALID"
        )
    if any(role) or membership_count != 0:
        raise PostgreSQLStagingPreflightError(
            "AURA_STAGING_ROLE_PRIVILEGES_INVALID"
        )
    if database_owner != EXPECTED_OWNER:
        raise PostgreSQLStagingPreflightError(
            "AURA_STAGING_DATABASE_OWNER_INVALID"
        )
    ...
    if tuple(bool(item) for item in table_privileges) != (True, False):
        raise PostgreSQLStagingPreflightError(
            "AURA_STAGING_TABLE_PRIVILEGES_INVALID"
        )
    if not sequence_privileges:
        raise PostgreSQLStagingPreflightError(
            "AURA_STAGING_SEQUENCE_PRIVILEGES_INVALID"
        )
    if any(forbidden_access.values()):
        raise PostgreSQLStagingPreflightError(
            "AURA_STAGING_CROSS_DATABASE_ACCESS"
        )
    print("least-privilege role checks: passed")
    print("test/public database access: rejected")
    print("AURA_POSTGRESQL_STAGING_PREFLIGHT_OK")


def main() -> int:
    try:
        run_preflight()
    except PostgreSQLStagingPreflightError as error:
        print(str(error), file=sys.stderr)
        return 2
    except Exception:
        print("AURA_STAGING_PREFLIGHT_FAILED", file=sys.stderr)
        return 2
    return 0
