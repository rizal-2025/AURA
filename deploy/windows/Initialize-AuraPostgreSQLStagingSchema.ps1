...
if ($currentRoot -ne $repositoryRoot.TrimEnd('\')) {
    throw 'AURA_STAGING_SCHEMA_REPOSITORY_ROOT_REQUIRED'
}
...
$migrationUrl = (
    'postgresql+psycopg://aura_migration_owner@127.0.0.1:5432/aura_demo_staging'
)
...
function Set-AuraStagingSchemaCredentialAcl {
    ...
    if ($LASTEXITCODE -ne 0) { throw 'AURA_STAGING_SCHEMA_PGPASSFILE_ACL_FAILED' }
    Assert-AuraOperatorSecretAcl -Path $Path
}

function Invoke-AuraStagingSchemaOperation {
    param(
        [Parameter(Mandatory)]
...
$tempName = 'staging-migration.pgpass.' + [Guid]::NewGuid().ToString('N') + '.tmp'
...
try {
    ...
    try {
        ...
        if ([string]::IsNullOrEmpty($plainPassword) -or $plainPassword -match '[\x00\r\n]') {
            throw 'AURA_STAGING_MIGRATION_PASSWORD_INVALID'
        }
        $escapedPassword = $plainPassword.Replace('\', '\\').Replace(':', '\:')
        $entry = "127.0.0.1:5432:aura_demo_staging:aura_migration_owner:$escapedPassword"
    ...
    Set-AuraStagingSchemaCredentialAcl -Path $tempPath

    $plan = Invoke-AuraStagingSchemaOperation `
    ...
    if (
        ...
        throw 'AURA_STAGING_SCHEMA_NOT_EMPTY'
    }
    $applied = Invoke-AuraStagingSchemaOperation `
    ...
    if (
        ...
        throw 'AURA_STAGING_SCHEMA_APPLY_FAILED'
    }
    $verified = Invoke-AuraStagingSchemaOperation `
    ...
    if (
        ...
        throw 'AURA_STAGING_SCHEMA_VERIFY_FAILED'
    }
} finally {
    $plan = $null
    $applied = $null
...
Write-Output 'AURA_STAGING_SCHEMA_INITIALIZED'
