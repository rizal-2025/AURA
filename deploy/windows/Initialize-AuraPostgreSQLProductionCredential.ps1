...
if ($currentRoot -ne $repositoryRoot.TrimEnd('\')) {
    throw 'AURA_PRODUCTION_CREDENTIAL_REPOSITORY_ROOT_REQUIRED'
}

Assert-AuraOperatorSecretAcl -Path $script:AuraSecretRoot
$expectedPgPassPath = 'C:\ProgramData\AURA\secrets\production.pgpass'
$pgPassPath = Assert-AuraPathWithin `
    -Path (Join-Path $script:AuraSecretRoot 'production.pgpass') `
    -Root $script:AuraSecretRoot
if ($pgPassPath -ne $expectedPgPassPath) {
    throw 'AURA_PRODUCTION_PGPASSFILE_PATH_INVALID'
}

$previous = Import-AuraConfiguration -Profile production
try {
    if (
        $env:DEMO_DATABASE_URL -ne 'postgresql+psycopg://aura_public_runtime@127.0.0.1:5432/aura_demo_public' `
...
if ($credentialExists -and -not $ReplaceExisting) {
    throw 'AURA_PRODUCTION_PGPASSFILE_ALREADY_EXISTS'
}
if (-not $credentialExists -and $ReplaceExisting) {
    throw 'AURA_PRODUCTION_PGPASSFILE_MISSING_FOR_REPLACEMENT'
}
if ($credentialExists) {
    Assert-AuraOperatorSecretAcl -Path $pgPassPath
}

function Set-AuraProductionCredentialAcl {
    ...
    if ($LASTEXITCODE -ne 0) { throw 'AURA_PRODUCTION_PGPASSFILE_ACL_FAILED' }
    Assert-AuraOperatorSecretAcl -Path $Path
}

function Test-AuraProductionCredential {
    param([Parameter(Mandatory)][string]$CredentialPath)

    $startInfo = [System.Diagnostics.ProcessStartInfo]::new()
    $startInfo.FileName = Get-AuraPythonPath
    $startInfo.Arguments = '-B -m tools.postgresql_production_preflight'
    ...
    try {
        ...
        return (
            ...
            -and $standardOutput.Contains('AURA_POSTGRESQL_PRODUCTION_PREFLIGHT_OK') `
...
function Replace-AuraFileWithoutBackup {
    ...
    if ($null -eq $replaceMethod) {
        throw 'AURA_PRODUCTION_PGPASSFILE_REPLACE_UNAVAILABLE'
    }
    try {
        ...
        throw 'AURA_PRODUCTION_PGPASSFILE_REPLACE_FAILED'
    }
}

$tempName = 'production.pgpass.' + [Guid]::NewGuid().ToString('N') + '.tmp'
...
try {
    $securePassword = Read-Host `
        'Password for existing PostgreSQL role aura_public_runtime' -AsSecureString
    $bstr = [IntPtr]::Zero
    $plainPassword = $null
    ...
    try {
        ...
        if ([string]::IsNullOrEmpty($plainPassword) -or $plainPassword -match '[\x00\r\n]') {
            throw 'AURA_PRODUCTION_DATABASE_PASSWORD_INVALID'
        }
        $escapedPassword = $plainPassword.Replace('\', '\\').Replace(':', '\:')
        $entry = "127.0.0.1:5432:aura_demo_public:aura_public_runtime:$escapedPassword"
    ...
    Set-AuraProductionCredentialAcl -Path $tempPath
    if (-not (Test-AuraProductionCredential -CredentialPath $tempPath)) {
        throw 'AURA_PRODUCTION_CREDENTIAL_VALIDATION_FAILED'
    }
    if ($ReplaceExisting) {
...
if ($ReplaceExisting) {
    Write-Output 'AURA_PRODUCTION_PGPASSFILE_UPDATED'
} else {
    Write-Output 'AURA_PRODUCTION_PGPASSFILE_CREATED'
}
